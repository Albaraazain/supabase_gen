...
/// Realtime provider for all messages records
/// 
...
///     final messagesStream = ref.watch(realtimeMessagesProvider);
///     
///     return messagesStream.when(
///       data: (messages) => ListView.builder(
///         itemCount: messages.length,
///         itemBuilder: (ctx, i) => MessagesTile(
///           key: ValueKey('messages-${messages[i].id}'),
///           messages: messages[i],
///         ),
///       ),
...
/// }
/// ```
final realtimeMessagesProvider = StreamProvider<List<MessagesModel>>((ref) {
  final repository = ref.watch(messagesRepositoryProvider);
  
  AppLogger.debug('Starting realtime stream for all messages', loggerName: 'Provider');
  
  return repository
      .streamAll()
      .handleError((error, stackTrace) {
        AppLogger.error(
          'Realtime stream error for messages: $error',
          loggerName: 'Provider',
          error: error,
...
/// Realtime provider for a single messages by ID
/// 
...
///   
///   const MessagesDetailView({required this.messagesId});
///   
///   Widget build(BuildContext context, WidgetRef ref) {
///     final messagesStream = ref.watch(realtimeMessagesByIdProvider(messagesId));
///     
///     return messagesStream.when(
///       data: (messages) => messages != null
///         ? MessagesDetail(messages: messages)
///         : const Text('Messages not found'),
///       loading: () => const CircularProgressIndicator(),
///       error: (e, _) => ErrorWidget.withDetails(message: e.toString()),
///     );
///   }
/// }
/// ```
final realtimeMessagesByIdProvider = StreamProvider.family<MessagesModel?, String>((ref, id) {
  final repository = ref.watch(messagesRepositoryProvider);
  
  AppLogger.debug('Starting realtime stream for messages ID: $id', loggerName: 'Provider');
  
  return repository
      .streamById(id)
      .handleError((error, stackTrace) {
        AppLogger.error(
          'Realtime stream error for messages ID $id: $error',
          loggerName: 'Provider',
          error: error,
          stackTrace: stackTrace,
        );
...
/// Realtime provider for filtered messages records with client-side filtering
/// 
...
/// 
/// Note: For large tables, consider using the unfiltered realtimeMessagesProvider
/// and filtering in your widget to reduce processing overhead.
/// 
/// Example usage:
/// ```dart
/// class ActiveMessagesWidget extends ConsumerWidget {
///   Widget build(BuildContext context, WidgetRef ref) {
///     // Create stable filter reference to avoid provider recreation
///     final activeFilter = {'status': 'active'};
///     final activeMessagesStream = ref.watch(
///       realtimeFilteredMessagesProvider(activeFilter)
///     );
///     
///     return activeMessagesStream.when(
///       data: (activeMessages) => Column(
///         children: activeMessages
///           .map((messages) => MessagesCard(
///             key: ValueKey('active-messages-${messages.id}'),
///             messages: messages,
///           ))
///           .toList(),
///       ),
...
/// }
/// ```
final realtimeFilteredMessagesProvider = StreamProvider.family<List<MessagesModel>, Map<String, dynamic>>((ref, filters) {
  final repository = ref.watch(messagesRepositoryProvider);
  
  // Create a readable filter description for logging
  final filterDesc = filters.entries.map((e) => '${e.key}=${e.value}').join(', ');
  AppLogger.debug('Starting filtered realtime stream for messages with filters: $filterDesc', loggerName: 'Provider');
  
  return repository
      .streamWhere(filters)
      .handleError((error, stackTrace) {
        AppLogger.error(
          'Filtered realtime stream error for messages (filters: $filterDesc): $error',
          loggerName: 'Provider',
          error: error,
          stackTrace: stackTrace,
        );
...
/// Example usage:
/// ```dart
/// class ReliableMessagesWidget extends ConsumerWidget {
///   Widget build(BuildContext context, WidgetRef ref) {
///     final messagesData = ref.watch(hybridMessagesProvider);
///     
///     return messagesData.when(
///       data: (messages) => MessagesList(messages: messages),
///       loading: () => const MessagesSkeleton(),
///       error: (e, _) => MessagesErrorView(error: e),
///     );
///   }
/// }
/// ```
final hybridMessagesProvider = StreamProvider<List<MessagesModel>>((ref) {
  final repository = ref.watch(messagesRepositoryProvider);
  
  AppLogger.debug('Starting hybrid realtime/cached stream for messages', loggerName: 'Provider');
  
  return repository
      .streamAll()
      .handleError((error, stackTrace) async* {
        AppLogger.warning(
          'Realtime failed for messages, falling back to cached data: $error',
          loggerName: 'Provider',
          error: error,
          stackTrace: stackTrace,
        );
        ...
        try {
          ...
          AppLogger.debug('Successfully fell back to cached data for messages (${fallbackData.length} records)', loggerName: 'Provider');
          yield fallbackData;
        } catch (fallbackError, fallbackStack) {
          AppLogger.error(
            'Both realtime and fallback failed for messages: $fallbackError',
            loggerName: 'Provider',
            error: fallbackError,
