...
final jobsRepositoryProvider = Provider<JobsRepository>((ref) {
  AppLogger.debug('Creating JobsRepository instance', loggerName: 'Provider');
  return JobsRepository(Supabase.instance.client);
});

// Main provider for managing jobs data
final jobsProvider = StateNotifierProvider<JobsNotifier, AsyncValue<List<JobsModel>>>((ref) {
  final repository = ref.watch(jobsRepositoryProvider);
  AppLogger.debug('Creating JobsNotifier', loggerName: 'Provider');
...
// Provider to get a single jobs by ID with caching
final jobsByIdProvider = FutureProvider.family<JobsModel?, String>((ref, id) async {
  // Create a stable cache key for this ID lookup
  final cacheKey = 'jobs:id:$id';
  
  AppLogger.debug('jobsByIdProvider called with id: $id', loggerName: 'Provider');
  final repository = ref.watch(jobsRepositoryProvider);
  
  try {
    // Use the app cache to prevent redundant database calls
    final result = await AppCache().getOrFetch<JobsModel?>(
      cacheKey,
      () => repository.find(id),
    ...
    if (result == null) {
      AppLogger.warning('No job found with ID: $id', loggerName: 'Provider');
    } else {
      AppLogger.debug('Found job with ID: $id', loggerName: 'Provider');
    }
    
    return result;
  } catch (e, stackTrace) {
    final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'JobsById');
    throw AppException(message: errorMsg, originalError: e);
  }
});

// Provider to get filtered jobs with proper caching
final filteredJobsProvider = FutureProvider.family<List<JobsModel>, Map<String, dynamic>>((ref, filters) async {
  // Create a stable cache key from the filters
  final cacheKey = _createCacheKey(filters);
  
  AppLogger.debug('filteredJobsProvider called with key: $cacheKey', loggerName: 'Provider');
  final repository = ref.watch(jobsRepositoryProvider);
  ...
  try {
    ...
    final results = await AppCache().getOrFetch<List<JobsModel>>(
      cacheKey,
      () async {
    ...
    AppLogger.debug('filteredJobsProvider returned ${results.length} items for key: $cacheKey', loggerName: 'Provider');
    return results;
  } catch (e, stackTrace) {
    final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'FilteredJobs');
    throw AppException(message: errorMsg, originalError: e);
  }
});
...
/// Notifier class that handles jobs operations
class JobsNotifier extends StateNotifier<AsyncValue<List<JobsModel>>> {
  ...
  Future<void> _loadInitialData() async {
    try {
      AppLogger.debug('Loading initial jobs data', loggerName: 'Provider');
      final results = await _repository.findAll();
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('JobsNotifier', 'Loaded initial data', details: '${results.length} records');
      }
    } catch (e, stackTrace) {
      if (mounted) {
        state = AsyncValue.error(e, stackTrace);
        ProviderLogging.logStateChange('JobsNotifier', 'Failed to load initial data', error: e, stackTrace: stackTrace);
      }
    }
  }

  /// Fetch all jobs with basic sorting and filtering
  Future<List<JobsModel>> fetchAll({
    String? orderBy,
    bool ascending = true,
    ...
    try {
      ProviderLogging.logStateChange('JobsNotifier', 'Fetching data', 
      ...
      final List<JobsModel> results;
      if (cacheKey != null) {
        results = await AppCache().getOrFetch<List<JobsModel>>(
          cacheKey,
          () => _repository.findAll(
            orderBy: orderBy,
            ascending: ascending,
      ...
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('JobsNotifier', 'Data fetched', details: '${results.length} records');
      }
      ...
      final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'Jobs');
      throw AppException(message: errorMsg, originalError: e);
    }
  }
  
  /// Get a single record by ID with caching
  Future<JobsModel?> getById(String id) async {
    try {
      // Create a stable cache key
      final cacheKey = 'jobs:id:$id';
      
      ProviderLogging.logStateChange('JobsNotifier', 'Getting record by ID', details: 'id: $id');
  ...
  /// Create a new record
  Future<JobsModel> create(JobsModel model) async {
    try {
      ProviderLogging.logStateChange('JobsNotifier', 'Creating record');
      final result = await _repository.insert(model);
      
      // Clear any cached list results that might contain this entity
      AppLogger.debug('Clearing jobs list caches after create', loggerName: 'Provider');
      ...
      if (mounted) {
        ...
        ProviderLogging.logStateChange('JobsNotifier', 'Record created', details: 'id: ${result.id}');
      }
  ...
  Future<JobsModel?> update(JobsModel model) async {
    try {
      ...
      if (modelId.isEmpty) {
        const message = 'Cannot update job without ID';
        ProviderLogging.logStateChange('JobsNotifier', 'Update failed', details: message);
        throw AppException(message: message);
      }
      
      ProviderLogging.logStateChange('JobsNotifier', 'Updating record', details: 'id: $modelId');
      final result = await _repository.update(model);
      
      // Clear related caches
      if (result != null) {
        // Clear both the specific ID cache and list caches
        final idCacheKey = 'jobs:id:$modelId';
      ...
      if (result != null && mounted && state.hasValue) {
        ...
        if (index >= 0) {
          ...
          ProviderLogging.logStateChange('JobsNotifier', 'Record updated', details: 'id: $modelId');
        }
      }
  ...
  Future<void> delete(String id) async {
    try {
      ProviderLogging.logStateChange('JobsNotifier', 'Deleting record', details: 'id: $id');
      await _repository.delete(id);
      
      // Clear cache entries for this ID
      final idCacheKey = 'jobs:id:$id';
      AppLogger.debug('Clearing cache for key: $idCacheKey', loggerName: 'Provider');
      AppCache().remove(idCacheKey);
      ...
      if (mounted && state.hasValue) {
        ...
        ProviderLogging.logStateChange('JobsNotifier', 'Record deleted', details: 'id: $id');
      }
    } catch (e, stackTrace) {
      final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'Jobs');
      throw AppException(message: errorMsg, originalError: e);
  ...
  void _clearRelatedCaches(JobsModel entity) {
    final prefix = 'jobs:';
    AppLogger.debug('Clearing list caches with prefix: $prefix', loggerName: 'Provider');
  ...
  Future<void> refresh() async {
    try {
      ProviderLogging.logStateChange('JobsNotifier', 'Refreshing data');
      if (mounted) {
        state = const AsyncValue.loading();
      }
      ...
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('JobsNotifier', 'Data refreshed', details: '${results.length} records');
      }
    } catch (e, stackTrace) {
      if (mounted) {
        state = AsyncValue.error(e, stackTrace);
      }
      ProviderLogging.logStateChange('JobsNotifier', 'Refresh failed', error: e, stackTrace: stackTrace);
    }
  }
}
