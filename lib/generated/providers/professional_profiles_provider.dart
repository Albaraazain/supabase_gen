...
import '../repositories/professional_profiles_repository.dart';
...
final professionalProfilesRepositoryProvider = Provider<ProfessionalProfilesRepository>((ref) {
  ...
  return ProfessionalProfilesRepository(Supabase.instance.client);
});

// Main provider for managing professional_profiles data
final professionalProfilesProvider = StateNotifierProvider<ProfessionalProfilesNotifier, AsyncValue<List<ProfessionalProfilesModel>>>((ref) {
  final repository = ref.watch(professionalProfilesRepositoryProvider);
  AppLogger.debug('Creating ProfessionalProfilesNotifier', loggerName: 'Provider');
  return ProfessionalProfilesNotifier(repository);
});
...
String _createCacheKey(Map<String, dynamic> filters) {
  ...
  return 'professional_profiles:${parts.join(',')}';
}

// Provider to get a single professional_profiles by ID with caching
final professionalProfilesByIdProvider = FutureProvider.family<ProfessionalProfilesModel?, String>((ref, id) async {
  // Create a stable cache key for this ID lookup
  final cacheKey = 'professional_profiles:id:$id';
  
  AppLogger.debug('professionalProfilesByIdProvider called with id: $id', loggerName: 'Provider');
  final repository = ref.watch(professionalProfilesRepositoryProvider);
  
  try {
    // Use the app cache to prevent redundant database calls
    final result = await AppCache().getOrFetch<ProfessionalProfilesModel?>(
      cacheKey,
      () => repository.find(id),
    ...
    if (result == null) {
      AppLogger.warning('No professional_profil found with ID: $id', loggerName: 'Provider');
    } else {
      AppLogger.debug('Found professional_profil with ID: $id', loggerName: 'Provider');
    }
    
    return result;
  } catch (e, stackTrace) {
    final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'ProfessionalProfilesById');
    throw AppException(message: errorMsg, originalError: e);
  }
});

// Provider to get filtered professional_profiles with proper caching
final filteredProfessionalProfilesProvider = FutureProvider.family<List<ProfessionalProfilesModel>, Map<String, dynamic>>((ref, filters) async {
  // Create a stable cache key from the filters
  final cacheKey = _createCacheKey(filters);
  
  AppLogger.debug('filteredProfessionalProfilesProvider called with key: $cacheKey', loggerName: 'Provider');
  final repository = ref.watch(professionalProfilesRepositoryProvider);
  ...
  try {
    ...
    final results = await AppCache().getOrFetch<List<ProfessionalProfilesModel>>(
      cacheKey,
      () async {
    ...
    AppLogger.debug('filteredProfessionalProfilesProvider returned ${results.length} items for key: $cacheKey', loggerName: 'Provider');
    return results;
  } catch (e, stackTrace) {
    final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'FilteredProfessionalProfiles');
    throw AppException(message: errorMsg, originalError: e);
  }
});
...
/// Notifier class that handles professional_profiles operations
class ProfessionalProfilesNotifier extends StateNotifier<AsyncValue<List<ProfessionalProfilesModel>>> {
  final ProfessionalProfilesRepository _repository;

  ProfessionalProfilesNotifier(this._repository) : super(const AsyncValue.loading()) {
  ...
  Future<void> _loadInitialData() async {
    try {
      AppLogger.debug('Loading initial professional_profiles data', loggerName: 'Provider');
      final results = await _repository.findAll();
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('ProfessionalProfilesNotifier', 'Loaded initial data', details: '${results.length} records');
      }
    } catch (e, stackTrace) {
      if (mounted) {
        state = AsyncValue.error(e, stackTrace);
        ProviderLogging.logStateChange('ProfessionalProfilesNotifier', 'Failed to load initial data', error: e, stackTrace: stackTrace);
      }
    }
  }

  /// Fetch all professional_profiles with basic sorting and filtering
  Future<List<ProfessionalProfilesModel>> fetchAll({
    String? orderBy,
    bool ascending = true,
    ...
    try {
      ProviderLogging.logStateChange('ProfessionalProfilesNotifier', 'Fetching data', 
      ...
      final List<ProfessionalProfilesModel> results;
      if (cacheKey != null) {
        results = await AppCache().getOrFetch<List<ProfessionalProfilesModel>>(
          cacheKey,
          () => _repository.findAll(
            orderBy: orderBy,
            ascending: ascending,
      ...
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('ProfessionalProfilesNotifier', 'Data fetched', details: '${results.length} records');
      }
      ...
      final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'ProfessionalProfiles');
      throw AppException(message: errorMsg, originalError: e);
    }
  }
  
  /// Get a single record by ID with caching
  Future<ProfessionalProfilesModel?> getById(String id) async {
    try {
      // Create a stable cache key
      final cacheKey = 'professional_profiles:id:$id';
      
      ProviderLogging.logStateChange('ProfessionalProfilesNotifier', 'Getting record by ID', details: 'id: $id');
  ...
  /// Create a new record
  Future<ProfessionalProfilesModel> create(ProfessionalProfilesModel model) async {
    try {
      ProviderLogging.logStateChange('ProfessionalProfilesNotifier', 'Creating record');
      final result = await _repository.insert(model);
      
      // Clear any cached list results that might contain this entity
      AppLogger.debug('Clearing professional_profiles list caches after create', loggerName: 'Provider');
      ...
      if (mounted) {
        ...
        ProviderLogging.logStateChange('ProfessionalProfilesNotifier', 'Record created', details: 'id: ${result.id}');
      }
  ...
  Future<ProfessionalProfilesModel?> update(ProfessionalProfilesModel model) async {
    try {
      ...
      if (modelId.isEmpty) {
        const message = 'Cannot update professional_profil without ID';
        ProviderLogging.logStateChange('ProfessionalProfilesNotifier', 'Update failed', details: message);
        throw AppException(message: message);
      }
      
      ProviderLogging.logStateChange('ProfessionalProfilesNotifier', 'Updating record', details: 'id: $modelId');
      final result = await _repository.update(model);
      
      // Clear related caches
      if (result != null) {
        // Clear both the specific ID cache and list caches
        final idCacheKey = 'professional_profiles:id:$modelId';
      ...
      if (result != null && mounted && state.hasValue) {
        ...
        if (index >= 0) {
          ...
          ProviderLogging.logStateChange('ProfessionalProfilesNotifier', 'Record updated', details: 'id: $modelId');
        }
      }
  ...
  Future<void> delete(String id) async {
    try {
      ProviderLogging.logStateChange('ProfessionalProfilesNotifier', 'Deleting record', details: 'id: $id');
      await _repository.delete(id);
      
      // Clear cache entries for this ID
      final idCacheKey = 'professional_profiles:id:$id';
      AppLogger.debug('Clearing cache for key: $idCacheKey', loggerName: 'Provider');
      AppCache().remove(idCacheKey);
      
      // Clear list caches that might contain this entity (using a prefix)
      final prefix = 'professional_profiles:';
      ...
      if (mounted && state.hasValue) {
        ...
        ProviderLogging.logStateChange('ProfessionalProfilesNotifier', 'Record deleted', details: 'id: $id');
      }
    } catch (e, stackTrace) {
      final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'ProfessionalProfiles');
      throw AppException(message: errorMsg, originalError: e);
  ...
  void _clearRelatedCaches(ProfessionalProfilesModel entity) {
    final prefix = 'professional_profiles:';
    AppLogger.debug('Clearing list caches with prefix: $prefix', loggerName: 'Provider');
  ...
  Future<void> refresh() async {
    try {
      ProviderLogging.logStateChange('ProfessionalProfilesNotifier', 'Refreshing data');
      if (mounted) {
        state = const AsyncValue.loading();
      }
      ...
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('ProfessionalProfilesNotifier', 'Data refreshed', details: '${results.length} records');
      }
    } catch (e, stackTrace) {
      if (mounted) {
        state = AsyncValue.error(e, stackTrace);
      }
      ProviderLogging.logStateChange('ProfessionalProfilesNotifier', 'Refresh failed', error: e, stackTrace: stackTrace);
    }
  }
}
