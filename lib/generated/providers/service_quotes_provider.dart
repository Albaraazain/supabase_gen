...
import '../repositories/service_quotes_repository.dart';
...
final serviceQuotesRepositoryProvider = Provider<ServiceQuotesRepository>((ref) {
  ...
  return ServiceQuotesRepository(Supabase.instance.client);
});

// Main provider for managing service_quotes data
final serviceQuotesProvider = StateNotifierProvider<ServiceQuotesNotifier, AsyncValue<List<ServiceQuotesModel>>>((ref) {
  final repository = ref.watch(serviceQuotesRepositoryProvider);
  AppLogger.debug('Creating ServiceQuotesNotifier', loggerName: 'Provider');
  return ServiceQuotesNotifier(repository);
});
...
String _createCacheKey(Map<String, dynamic> filters) {
  ...
  return 'service_quotes:${parts.join(',')}';
}

// Provider to get a single service_quotes by ID with caching
final serviceQuotesByIdProvider = FutureProvider.family<ServiceQuotesModel?, String>((ref, id) async {
  // Create a stable cache key for this ID lookup
  final cacheKey = 'service_quotes:id:$id';
  
  AppLogger.debug('serviceQuotesByIdProvider called with id: $id', loggerName: 'Provider');
  final repository = ref.watch(serviceQuotesRepositoryProvider);
  
  try {
    // Use the app cache to prevent redundant database calls
    final result = await AppCache().getOrFetch<ServiceQuotesModel?>(
      cacheKey,
      () => repository.find(id),
    ...
    if (result == null) {
      AppLogger.warning('No service_quot found with ID: $id', loggerName: 'Provider');
    } else {
      AppLogger.debug('Found service_quot with ID: $id', loggerName: 'Provider');
    }
    
    return result;
  } catch (e, stackTrace) {
    final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'ServiceQuotesById');
    throw AppException(message: errorMsg, originalError: e);
  }
});

// Provider to get filtered service_quotes with proper caching
final filteredServiceQuotesProvider = FutureProvider.family<List<ServiceQuotesModel>, Map<String, dynamic>>((ref, filters) async {
  // Create a stable cache key from the filters
  final cacheKey = _createCacheKey(filters);
  
  AppLogger.debug('filteredServiceQuotesProvider called with key: $cacheKey', loggerName: 'Provider');
  final repository = ref.watch(serviceQuotesRepositoryProvider);
  ...
  try {
    ...
    final results = await AppCache().getOrFetch<List<ServiceQuotesModel>>(
      cacheKey,
      () async {
    ...
    AppLogger.debug('filteredServiceQuotesProvider returned ${results.length} items for key: $cacheKey', loggerName: 'Provider');
    return results;
  } catch (e, stackTrace) {
    final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'FilteredServiceQuotes');
    throw AppException(message: errorMsg, originalError: e);
  }
});
...
/// Notifier class that handles service_quotes operations
class ServiceQuotesNotifier extends StateNotifier<AsyncValue<List<ServiceQuotesModel>>> {
  final ServiceQuotesRepository _repository;

  ServiceQuotesNotifier(this._repository) : super(const AsyncValue.loading()) {
  ...
  Future<void> _loadInitialData() async {
    try {
      AppLogger.debug('Loading initial service_quotes data', loggerName: 'Provider');
      final results = await _repository.findAll();
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('ServiceQuotesNotifier', 'Loaded initial data', details: '${results.length} records');
      }
    } catch (e, stackTrace) {
      if (mounted) {
        state = AsyncValue.error(e, stackTrace);
        ProviderLogging.logStateChange('ServiceQuotesNotifier', 'Failed to load initial data', error: e, stackTrace: stackTrace);
      }
    }
  }

  /// Fetch all service_quotes with basic sorting and filtering
  Future<List<ServiceQuotesModel>> fetchAll({
    String? orderBy,
    bool ascending = true,
    ...
    try {
      ProviderLogging.logStateChange('ServiceQuotesNotifier', 'Fetching data', 
      ...
      final List<ServiceQuotesModel> results;
      if (cacheKey != null) {
        results = await AppCache().getOrFetch<List<ServiceQuotesModel>>(
          cacheKey,
          () => _repository.findAll(
            orderBy: orderBy,
            ascending: ascending,
      ...
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('ServiceQuotesNotifier', 'Data fetched', details: '${results.length} records');
      }
      ...
      final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'ServiceQuotes');
      throw AppException(message: errorMsg, originalError: e);
    }
  }
  
  /// Get a single record by ID with caching
  Future<ServiceQuotesModel?> getById(String id) async {
    try {
      // Create a stable cache key
      final cacheKey = 'service_quotes:id:$id';
      
      ProviderLogging.logStateChange('ServiceQuotesNotifier', 'Getting record by ID', details: 'id: $id');
  ...
  /// Create a new record
  Future<ServiceQuotesModel> create(ServiceQuotesModel model) async {
    try {
      ProviderLogging.logStateChange('ServiceQuotesNotifier', 'Creating record');
      final result = await _repository.insert(model);
      
      // Clear any cached list results that might contain this entity
      AppLogger.debug('Clearing service_quotes list caches after create', loggerName: 'Provider');
      ...
      if (mounted) {
        ...
        ProviderLogging.logStateChange('ServiceQuotesNotifier', 'Record created', details: 'id: ${result.id}');
      }
  ...
  Future<ServiceQuotesModel?> update(ServiceQuotesModel model) async {
    try {
      ...
      if (modelId.isEmpty) {
        const message = 'Cannot update service_quot without ID';
        ProviderLogging.logStateChange('ServiceQuotesNotifier', 'Update failed', details: message);
        throw AppException(message: message);
      }
      
      ProviderLogging.logStateChange('ServiceQuotesNotifier', 'Updating record', details: 'id: $modelId');
      final result = await _repository.update(model);
      
      // Clear related caches
      if (result != null) {
        // Clear both the specific ID cache and list caches
        final idCacheKey = 'service_quotes:id:$modelId';
      ...
      if (result != null && mounted && state.hasValue) {
        ...
        if (index >= 0) {
          ...
          ProviderLogging.logStateChange('ServiceQuotesNotifier', 'Record updated', details: 'id: $modelId');
        }
      }
  ...
  Future<void> delete(String id) async {
    try {
      ProviderLogging.logStateChange('ServiceQuotesNotifier', 'Deleting record', details: 'id: $id');
      await _repository.delete(id);
      
      // Clear cache entries for this ID
      final idCacheKey = 'service_quotes:id:$id';
      AppLogger.debug('Clearing cache for key: $idCacheKey', loggerName: 'Provider');
      AppCache().remove(idCacheKey);
      
      // Clear list caches that might contain this entity (using a prefix)
      final prefix = 'service_quotes:';
      ...
      if (mounted && state.hasValue) {
        ...
        ProviderLogging.logStateChange('ServiceQuotesNotifier', 'Record deleted', details: 'id: $id');
      }
    } catch (e, stackTrace) {
      final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'ServiceQuotes');
      throw AppException(message: errorMsg, originalError: e);
  ...
  void _clearRelatedCaches(ServiceQuotesModel entity) {
    final prefix = 'service_quotes:';
    AppLogger.debug('Clearing list caches with prefix: $prefix', loggerName: 'Provider');
  ...
  Future<void> refresh() async {
    try {
      ProviderLogging.logStateChange('ServiceQuotesNotifier', 'Refreshing data');
      if (mounted) {
        state = const AsyncValue.loading();
      }
      ...
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('ServiceQuotesNotifier', 'Data refreshed', details: '${results.length} records');
      }
    } catch (e, stackTrace) {
      if (mounted) {
        state = AsyncValue.error(e, stackTrace);
      }
      ProviderLogging.logStateChange('ServiceQuotesNotifier', 'Refresh failed', error: e, stackTrace: stackTrace);
    }
  }
}
