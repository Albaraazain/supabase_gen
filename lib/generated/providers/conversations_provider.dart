...
final conversationsRepositoryProvider = Provider<ConversationsRepository>((ref) {
  AppLogger.debug('Creating ConversationsRepository instance', loggerName: 'Provider');
  return ConversationsRepository(Supabase.instance.client);
});

// Main provider for managing conversations data
final conversationsProvider = StateNotifierProvider<ConversationsNotifier, AsyncValue<List<ConversationsModel>>>((ref) {
  final repository = ref.watch(conversationsRepositoryProvider);
  AppLogger.debug('Creating ConversationsNotifier', loggerName: 'Provider');
...
// Provider to get a single conversations by ID with caching
final conversationsByIdProvider = FutureProvider.family<ConversationsModel?, String>((ref, id) async {
  // Create a stable cache key for this ID lookup
  final cacheKey = 'conversations:id:$id';
  
  AppLogger.debug('conversationsByIdProvider called with id: $id', loggerName: 'Provider');
  final repository = ref.watch(conversationsRepositoryProvider);
  
  try {
    // Use the app cache to prevent redundant database calls
    final result = await AppCache().getOrFetch<ConversationsModel?>(
      cacheKey,
      () => repository.find(id),
    ...
    if (result == null) {
      AppLogger.warning('No conversation found with ID: $id', loggerName: 'Provider');
    } else {
      AppLogger.debug('Found conversation with ID: $id', loggerName: 'Provider');
    }
    
    return result;
  } catch (e, stackTrace) {
    final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'ConversationsById');
    throw AppException(message: errorMsg, originalError: e);
  }
});

// Provider to get filtered conversations with proper caching
final filteredConversationsProvider = FutureProvider.family<List<ConversationsModel>, Map<String, dynamic>>((ref, filters) async {
  // Create a stable cache key from the filters
  final cacheKey = _createCacheKey(filters);
  
  AppLogger.debug('filteredConversationsProvider called with key: $cacheKey', loggerName: 'Provider');
  final repository = ref.watch(conversationsRepositoryProvider);
  ...
  try {
    ...
    final results = await AppCache().getOrFetch<List<ConversationsModel>>(
      cacheKey,
      () async {
    ...
    AppLogger.debug('filteredConversationsProvider returned ${results.length} items for key: $cacheKey', loggerName: 'Provider');
    return results;
  } catch (e, stackTrace) {
    final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'FilteredConversations');
    throw AppException(message: errorMsg, originalError: e);
  }
});
...
/// Notifier class that handles conversations operations
class ConversationsNotifier extends StateNotifier<AsyncValue<List<ConversationsModel>>> {
  ...
  Future<void> _loadInitialData() async {
    try {
      AppLogger.debug('Loading initial conversations data', loggerName: 'Provider');
      final results = await _repository.findAll();
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('ConversationsNotifier', 'Loaded initial data', details: '${results.length} records');
      }
    } catch (e, stackTrace) {
      if (mounted) {
        state = AsyncValue.error(e, stackTrace);
        ProviderLogging.logStateChange('ConversationsNotifier', 'Failed to load initial data', error: e, stackTrace: stackTrace);
      }
    }
  }

  /// Fetch all conversations with basic sorting and filtering
  Future<List<ConversationsModel>> fetchAll({
    String? orderBy,
    bool ascending = true,
    ...
    try {
      ProviderLogging.logStateChange('ConversationsNotifier', 'Fetching data', 
      ...
      final List<ConversationsModel> results;
      if (cacheKey != null) {
        results = await AppCache().getOrFetch<List<ConversationsModel>>(
          cacheKey,
          () => _repository.findAll(
            orderBy: orderBy,
            ascending: ascending,
      ...
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('ConversationsNotifier', 'Data fetched', details: '${results.length} records');
      }
      ...
      final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'Conversations');
      throw AppException(message: errorMsg, originalError: e);
    }
  }
  
  /// Get a single record by ID with caching
  Future<ConversationsModel?> getById(String id) async {
    try {
      // Create a stable cache key
      final cacheKey = 'conversations:id:$id';
      
      ProviderLogging.logStateChange('ConversationsNotifier', 'Getting record by ID', details: 'id: $id');
  ...
  /// Create a new record
  Future<ConversationsModel> create(ConversationsModel model) async {
    try {
      ProviderLogging.logStateChange('ConversationsNotifier', 'Creating record');
      final result = await _repository.insert(model);
      
      // Clear any cached list results that might contain this entity
      AppLogger.debug('Clearing conversations list caches after create', loggerName: 'Provider');
      ...
      if (mounted) {
        ...
        ProviderLogging.logStateChange('ConversationsNotifier', 'Record created', details: 'id: ${result.id}');
      }
  ...
  Future<ConversationsModel?> update(ConversationsModel model) async {
    try {
      final modelId = model.id;
      if (modelId.isEmpty) {
        const message = 'Cannot update conversation without ID';
        ProviderLogging.logStateChange('ConversationsNotifier', 'Update failed', details: message);
        throw AppException(message: message);
      }
      
      ProviderLogging.logStateChange('ConversationsNotifier', 'Updating record', details: 'id: $modelId');
      final result = await _repository.update(model);
      
      // Clear related caches
      if (result != null) {
        // Clear both the specific ID cache and list caches
        final idCacheKey = 'conversations:id:$modelId';
      ...
      if (result != null && mounted && state.hasValue) {
        ...
        if (index >= 0) {
          ...
          ProviderLogging.logStateChange('ConversationsNotifier', 'Record updated', details: 'id: $modelId');
        }
      }
  ...
  Future<void> delete(String id) async {
    try {
      ProviderLogging.logStateChange('ConversationsNotifier', 'Deleting record', details: 'id: $id');
      await _repository.delete(id);
      
      // Clear cache entries for this ID
      final idCacheKey = 'conversations:id:$id';
      AppLogger.debug('Clearing cache for key: $idCacheKey', loggerName: 'Provider');
      AppCache().remove(idCacheKey);
      
      // Clear list caches that might contain this entity (using a prefix)
      final prefix = 'conversations:';
      ...
      if (mounted && state.hasValue) {
        ...
        ProviderLogging.logStateChange('ConversationsNotifier', 'Record deleted', details: 'id: $id');
      }
    } catch (e, stackTrace) {
      final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'Conversations');
      throw AppException(message: errorMsg, originalError: e);
  ...
  void _clearRelatedCaches(ConversationsModel entity) {
    final prefix = 'conversations:';
    AppLogger.debug('Clearing list caches with prefix: $prefix', loggerName: 'Provider');
  ...
  Future<void> refresh() async {
    try {
      ProviderLogging.logStateChange('ConversationsNotifier', 'Refreshing data');
      if (mounted) {
        state = const AsyncValue.loading();
      }
      ...
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('ConversationsNotifier', 'Data refreshed', details: '${results.length} records');
      }
    } catch (e, stackTrace) {
      if (mounted) {
        state = AsyncValue.error(e, stackTrace);
      }
      ProviderLogging.logStateChange('ConversationsNotifier', 'Refresh failed', error: e, stackTrace: stackTrace);
    }
  }
}
