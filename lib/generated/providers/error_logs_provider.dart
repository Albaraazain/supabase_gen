...
import '../repositories/error_logs_repository.dart';
...
final errorLogsRepositoryProvider = Provider<ErrorLogsRepository>((ref) {
  AppLogger.debug('Creating ErrorLogsRepository instance', loggerName: 'Provider');
  return ErrorLogsRepository(Supabase.instance.client);
});

// Main provider for managing error_logs data
final errorLogsProvider = StateNotifierProvider<ErrorLogsNotifier, AsyncValue<List<ErrorLogsModel>>>((ref) {
  final repository = ref.watch(errorLogsRepositoryProvider);
  AppLogger.debug('Creating ErrorLogsNotifier', loggerName: 'Provider');
  return ErrorLogsNotifier(repository);
});
...
String _createCacheKey(Map<String, dynamic> filters) {
  ...
  return 'error_logs:${parts.join(',')}';
}

// Provider to get a single error_logs by ID with caching
final errorLogsByIdProvider = FutureProvider.family<ErrorLogsModel?, String>((ref, id) async {
  // Create a stable cache key for this ID lookup
  final cacheKey = 'error_logs:id:$id';
  
  AppLogger.debug('errorLogsByIdProvider called with id: $id', loggerName: 'Provider');
  final repository = ref.watch(errorLogsRepositoryProvider);
  
  try {
    // Use the app cache to prevent redundant database calls
    final result = await AppCache().getOrFetch<ErrorLogsModel?>(
      cacheKey,
      () => repository.find(id),
    ...
    if (result == null) {
      AppLogger.warning('No error_log found with ID: $id', loggerName: 'Provider');
    } else {
      AppLogger.debug('Found error_log with ID: $id', loggerName: 'Provider');
    }
    
    return result;
  } catch (e, stackTrace) {
    final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'ErrorLogsById');
    throw AppException(message: errorMsg, originalError: e);
  }
});

// Provider to get filtered error_logs with proper caching
final filteredErrorLogsProvider = FutureProvider.family<List<ErrorLogsModel>, Map<String, dynamic>>((ref, filters) async {
  // Create a stable cache key from the filters
  final cacheKey = _createCacheKey(filters);
  
  AppLogger.debug('filteredErrorLogsProvider called with key: $cacheKey', loggerName: 'Provider');
  final repository = ref.watch(errorLogsRepositoryProvider);
  ...
  try {
    ...
    final results = await AppCache().getOrFetch<List<ErrorLogsModel>>(
      cacheKey,
      () async {
    ...
    AppLogger.debug('filteredErrorLogsProvider returned ${results.length} items for key: $cacheKey', loggerName: 'Provider');
    return results;
  } catch (e, stackTrace) {
    final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'FilteredErrorLogs');
    throw AppException(message: errorMsg, originalError: e);
  }
});
...
/// Notifier class that handles error_logs operations
class ErrorLogsNotifier extends StateNotifier<AsyncValue<List<ErrorLogsModel>>> {
  final ErrorLogsRepository _repository;

  ErrorLogsNotifier(this._repository) : super(const AsyncValue.loading()) {
  ...
  Future<void> _loadInitialData() async {
    try {
      AppLogger.debug('Loading initial error_logs data', loggerName: 'Provider');
      final results = await _repository.findAll();
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('ErrorLogsNotifier', 'Loaded initial data', details: '${results.length} records');
      }
    } catch (e, stackTrace) {
      if (mounted) {
        state = AsyncValue.error(e, stackTrace);
        ProviderLogging.logStateChange('ErrorLogsNotifier', 'Failed to load initial data', error: e, stackTrace: stackTrace);
      }
    }
  }

  /// Fetch all error_logs with basic sorting and filtering
  Future<List<ErrorLogsModel>> fetchAll({
    String? orderBy,
    bool ascending = true,
    ...
    try {
      ProviderLogging.logStateChange('ErrorLogsNotifier', 'Fetching data', 
      ...
      final List<ErrorLogsModel> results;
      if (cacheKey != null) {
        results = await AppCache().getOrFetch<List<ErrorLogsModel>>(
          cacheKey,
          () => _repository.findAll(
            orderBy: orderBy,
            ascending: ascending,
      ...
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('ErrorLogsNotifier', 'Data fetched', details: '${results.length} records');
      }
      ...
      final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'ErrorLogs');
      throw AppException(message: errorMsg, originalError: e);
    }
  }
  
  /// Get a single record by ID with caching
  Future<ErrorLogsModel?> getById(String id) async {
    try {
      // Create a stable cache key
      final cacheKey = 'error_logs:id:$id';
      
      ProviderLogging.logStateChange('ErrorLogsNotifier', 'Getting record by ID', details: 'id: $id');
  ...
  /// Create a new record
  Future<ErrorLogsModel> create(ErrorLogsModel model) async {
    try {
      ProviderLogging.logStateChange('ErrorLogsNotifier', 'Creating record');
      final result = await _repository.insert(model);
      
      // Clear any cached list results that might contain this entity
      AppLogger.debug('Clearing error_logs list caches after create', loggerName: 'Provider');
      ...
      if (mounted) {
        ...
        ProviderLogging.logStateChange('ErrorLogsNotifier', 'Record created', details: 'id: ${result.id}');
      }
  ...
  Future<ErrorLogsModel?> update(ErrorLogsModel model) async {
    try {
      ...
      if (modelId.isEmpty) {
        const message = 'Cannot update error_log without ID';
        ProviderLogging.logStateChange('ErrorLogsNotifier', 'Update failed', details: message);
        throw AppException(message: message);
      }
      
      ProviderLogging.logStateChange('ErrorLogsNotifier', 'Updating record', details: 'id: $modelId');
      final result = await _repository.update(model);
      
      // Clear related caches
      if (result != null) {
        // Clear both the specific ID cache and list caches
        final idCacheKey = 'error_logs:id:$modelId';
      ...
      if (result != null && mounted && state.hasValue) {
        ...
        if (index >= 0) {
          ...
          ProviderLogging.logStateChange('ErrorLogsNotifier', 'Record updated', details: 'id: $modelId');
        }
      }
  ...
  Future<void> delete(String id) async {
    try {
      ProviderLogging.logStateChange('ErrorLogsNotifier', 'Deleting record', details: 'id: $id');
      await _repository.delete(id);
      
      // Clear cache entries for this ID
      final idCacheKey = 'error_logs:id:$id';
      AppLogger.debug('Clearing cache for key: $idCacheKey', loggerName: 'Provider');
      AppCache().remove(idCacheKey);
      
      // Clear list caches that might contain this entity (using a prefix)
      final prefix = 'error_logs:';
      ...
      if (mounted && state.hasValue) {
        ...
        ProviderLogging.logStateChange('ErrorLogsNotifier', 'Record deleted', details: 'id: $id');
      }
    } catch (e, stackTrace) {
      final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'ErrorLogs');
      throw AppException(message: errorMsg, originalError: e);
  ...
  void _clearRelatedCaches(ErrorLogsModel entity) {
    final prefix = 'error_logs:';
    AppLogger.debug('Clearing list caches with prefix: $prefix', loggerName: 'Provider');
  ...
  Future<void> refresh() async {
    try {
      ProviderLogging.logStateChange('ErrorLogsNotifier', 'Refreshing data');
      if (mounted) {
        state = const AsyncValue.loading();
      }
      ...
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('ErrorLogsNotifier', 'Data refreshed', details: '${results.length} records');
      }
    } catch (e, stackTrace) {
      if (mounted) {
        state = AsyncValue.error(e, stackTrace);
      }
      ProviderLogging.logStateChange('ErrorLogsNotifier', 'Refresh failed', error: e, stackTrace: stackTrace);
    }
  }
}
