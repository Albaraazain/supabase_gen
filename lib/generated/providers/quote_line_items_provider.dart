...
import '../repositories/quote_line_items_repository.dart';
...
final quoteLineItemsRepositoryProvider = Provider<QuoteLineItemsRepository>((ref) {
  ...
  return QuoteLineItemsRepository(Supabase.instance.client);
});

// Main provider for managing quote_line_items data
final quoteLineItemsProvider = StateNotifierProvider<QuoteLineItemsNotifier, AsyncValue<List<QuoteLineItemsModel>>>((ref) {
  final repository = ref.watch(quoteLineItemsRepositoryProvider);
  AppLogger.debug('Creating QuoteLineItemsNotifier', loggerName: 'Provider');
  return QuoteLineItemsNotifier(repository);
});
...
String _createCacheKey(Map<String, dynamic> filters) {
  ...
  return 'quote_line_items:${parts.join(',')}';
}

// Provider to get a single quote_line_items by ID with caching
final quoteLineItemsByIdProvider = FutureProvider.family<QuoteLineItemsModel?, String>((ref, id) async {
  // Create a stable cache key for this ID lookup
  final cacheKey = 'quote_line_items:id:$id';
  
  AppLogger.debug('quoteLineItemsByIdProvider called with id: $id', loggerName: 'Provider');
  final repository = ref.watch(quoteLineItemsRepositoryProvider);
  
  try {
    // Use the app cache to prevent redundant database calls
    final result = await AppCache().getOrFetch<QuoteLineItemsModel?>(
      cacheKey,
      () => repository.find(id),
    ...
    if (result == null) {
      AppLogger.warning('No quote_line_item found with ID: $id', loggerName: 'Provider');
    } else {
      AppLogger.debug('Found quote_line_item with ID: $id', loggerName: 'Provider');
    }
    
    return result;
  } catch (e, stackTrace) {
    final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'QuoteLineItemsById');
    throw AppException(message: errorMsg, originalError: e);
  }
});

// Provider to get filtered quote_line_items with proper caching
final filteredQuoteLineItemsProvider = FutureProvider.family<List<QuoteLineItemsModel>, Map<String, dynamic>>((ref, filters) async {
  // Create a stable cache key from the filters
  final cacheKey = _createCacheKey(filters);
  
  AppLogger.debug('filteredQuoteLineItemsProvider called with key: $cacheKey', loggerName: 'Provider');
  final repository = ref.watch(quoteLineItemsRepositoryProvider);
  ...
  try {
    ...
    final results = await AppCache().getOrFetch<List<QuoteLineItemsModel>>(
      cacheKey,
      () async {
    ...
    AppLogger.debug('filteredQuoteLineItemsProvider returned ${results.length} items for key: $cacheKey', loggerName: 'Provider');
    return results;
  } catch (e, stackTrace) {
    final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'FilteredQuoteLineItems');
    throw AppException(message: errorMsg, originalError: e);
  }
});
...
/// Notifier class that handles quote_line_items operations
class QuoteLineItemsNotifier extends StateNotifier<AsyncValue<List<QuoteLineItemsModel>>> {
  final QuoteLineItemsRepository _repository;

  QuoteLineItemsNotifier(this._repository) : super(const AsyncValue.loading()) {
  ...
  Future<void> _loadInitialData() async {
    try {
      AppLogger.debug('Loading initial quote_line_items data', loggerName: 'Provider');
      final results = await _repository.findAll();
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('QuoteLineItemsNotifier', 'Loaded initial data', details: '${results.length} records');
      }
    } catch (e, stackTrace) {
      if (mounted) {
        state = AsyncValue.error(e, stackTrace);
        ProviderLogging.logStateChange('QuoteLineItemsNotifier', 'Failed to load initial data', error: e, stackTrace: stackTrace);
      }
    }
  }

  /// Fetch all quote_line_items with basic sorting and filtering
  Future<List<QuoteLineItemsModel>> fetchAll({
    String? orderBy,
    bool ascending = true,
    ...
    try {
      ProviderLogging.logStateChange('QuoteLineItemsNotifier', 'Fetching data', 
      ...
      final List<QuoteLineItemsModel> results;
      if (cacheKey != null) {
        results = await AppCache().getOrFetch<List<QuoteLineItemsModel>>(
          cacheKey,
          () => _repository.findAll(
            orderBy: orderBy,
            ascending: ascending,
      ...
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('QuoteLineItemsNotifier', 'Data fetched', details: '${results.length} records');
      }
      ...
      final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'QuoteLineItems');
      throw AppException(message: errorMsg, originalError: e);
    }
  }
  
  /// Get a single record by ID with caching
  Future<QuoteLineItemsModel?> getById(String id) async {
    try {
      // Create a stable cache key
      final cacheKey = 'quote_line_items:id:$id';
      
      ProviderLogging.logStateChange('QuoteLineItemsNotifier', 'Getting record by ID', details: 'id: $id');
  ...
  /// Create a new record
  Future<QuoteLineItemsModel> create(QuoteLineItemsModel model) async {
    try {
      ProviderLogging.logStateChange('QuoteLineItemsNotifier', 'Creating record');
      final result = await _repository.insert(model);
      
      // Clear any cached list results that might contain this entity
      AppLogger.debug('Clearing quote_line_items list caches after create', loggerName: 'Provider');
      ...
      if (mounted) {
        ...
        ProviderLogging.logStateChange('QuoteLineItemsNotifier', 'Record created', details: 'id: ${result.id}');
      }
  ...
  Future<QuoteLineItemsModel?> update(QuoteLineItemsModel model) async {
    try {
      ...
      if (modelId.isEmpty) {
        const message = 'Cannot update quote_line_item without ID';
        ProviderLogging.logStateChange('QuoteLineItemsNotifier', 'Update failed', details: message);
        throw AppException(message: message);
      }
      
      ProviderLogging.logStateChange('QuoteLineItemsNotifier', 'Updating record', details: 'id: $modelId');
      final result = await _repository.update(model);
      
      // Clear related caches
      if (result != null) {
        // Clear both the specific ID cache and list caches
        final idCacheKey = 'quote_line_items:id:$modelId';
      ...
      if (result != null && mounted && state.hasValue) {
        ...
        if (index >= 0) {
          ...
          ProviderLogging.logStateChange('QuoteLineItemsNotifier', 'Record updated', details: 'id: $modelId');
        }
      }
  ...
  Future<void> delete(String id) async {
    try {
      ProviderLogging.logStateChange('QuoteLineItemsNotifier', 'Deleting record', details: 'id: $id');
      await _repository.delete(id);
      
      // Clear cache entries for this ID
      final idCacheKey = 'quote_line_items:id:$id';
      AppLogger.debug('Clearing cache for key: $idCacheKey', loggerName: 'Provider');
      AppCache().remove(idCacheKey);
      
      // Clear list caches that might contain this entity (using a prefix)
      final prefix = 'quote_line_items:';
      ...
      if (mounted && state.hasValue) {
        ...
        ProviderLogging.logStateChange('QuoteLineItemsNotifier', 'Record deleted', details: 'id: $id');
      }
    } catch (e, stackTrace) {
      final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'QuoteLineItems');
      throw AppException(message: errorMsg, originalError: e);
  ...
  void _clearRelatedCaches(QuoteLineItemsModel entity) {
    final prefix = 'quote_line_items:';
    AppLogger.debug('Clearing list caches with prefix: $prefix', loggerName: 'Provider');
  ...
  Future<void> refresh() async {
    try {
      ProviderLogging.logStateChange('QuoteLineItemsNotifier', 'Refreshing data');
      if (mounted) {
        state = const AsyncValue.loading();
      }
      ...
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('QuoteLineItemsNotifier', 'Data refreshed', details: '${results.length} records');
      }
    } catch (e, stackTrace) {
      if (mounted) {
        state = AsyncValue.error(e, stackTrace);
      }
      ProviderLogging.logStateChange('QuoteLineItemsNotifier', 'Refresh failed', error: e, stackTrace: stackTrace);
    }
  }
}
