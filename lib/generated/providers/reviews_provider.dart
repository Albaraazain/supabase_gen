...
final reviewsRepositoryProvider = Provider<ReviewsRepository>((ref) {
  AppLogger.debug('Creating ReviewsRepository instance', loggerName: 'Provider');
  return ReviewsRepository(Supabase.instance.client);
});

// Main provider for managing reviews data
final reviewsProvider = StateNotifierProvider<ReviewsNotifier, AsyncValue<List<ReviewsModel>>>((ref) {
  final repository = ref.watch(reviewsRepositoryProvider);
  AppLogger.debug('Creating ReviewsNotifier', loggerName: 'Provider');
...
// Provider to get a single reviews by ID with caching
final reviewsByIdProvider = FutureProvider.family<ReviewsModel?, String>((ref, id) async {
  // Create a stable cache key for this ID lookup
  final cacheKey = 'reviews:id:$id';
  
  AppLogger.debug('reviewsByIdProvider called with id: $id', loggerName: 'Provider');
  final repository = ref.watch(reviewsRepositoryProvider);
  
  try {
    // Use the app cache to prevent redundant database calls
    final result = await AppCache().getOrFetch<ReviewsModel?>(
      cacheKey,
      () => repository.find(id),
    ...
    if (result == null) {
      AppLogger.warning('No review found with ID: $id', loggerName: 'Provider');
    } else {
      AppLogger.debug('Found review with ID: $id', loggerName: 'Provider');
    }
    
    return result;
  } catch (e, stackTrace) {
    final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'ReviewsById');
    throw AppException(message: errorMsg, originalError: e);
  }
});

// Provider to get filtered reviews with proper caching
final filteredReviewsProvider = FutureProvider.family<List<ReviewsModel>, Map<String, dynamic>>((ref, filters) async {
  // Create a stable cache key from the filters
  final cacheKey = _createCacheKey(filters);
  
  AppLogger.debug('filteredReviewsProvider called with key: $cacheKey', loggerName: 'Provider');
  final repository = ref.watch(reviewsRepositoryProvider);
  ...
  try {
    ...
    final results = await AppCache().getOrFetch<List<ReviewsModel>>(
      cacheKey,
      () async {
    ...
    AppLogger.debug('filteredReviewsProvider returned ${results.length} items for key: $cacheKey', loggerName: 'Provider');
    return results;
  } catch (e, stackTrace) {
    final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'FilteredReviews');
    throw AppException(message: errorMsg, originalError: e);
  }
});
...
/// Realtime provider for all reviews records
/// 
...
///     final reviewsStream = ref.watch(realtimeReviewsProvider);
///     
///     return reviewsStream.when(
///       data: (reviews) => ListView.builder(
///         itemCount: reviews.length,
///         itemBuilder: (ctx, i) => ReviewsTile(
///           key: ValueKey('reviews-${reviews[i].id}'),
///           reviews: reviews[i],
///         ),
///       ),
...
/// }
/// ```
final realtimeReviewsProvider = StreamProvider<List<ReviewsModel>>((ref) {
  final repository = ref.watch(reviewsRepositoryProvider);
  
  AppLogger.debug('Starting realtime stream for all reviews', loggerName: 'Provider');
  
  return repository
      .streamAll()
      .handleError((error, stackTrace) {
        AppLogger.error(
          'Realtime stream error for reviews: $error',
          loggerName: 'Provider',
          error: error,
...
/// Realtime provider for a single reviews by ID
/// 
...
/// Example usage:
/// ```dart
/// class ReviewsDetailView extends ConsumerWidget {
///   final String reviewsId;
///   
///   const ReviewsDetailView({required this.reviewsId});
///   
///   Widget build(BuildContext context, WidgetRef ref) {
///     final reviewsStream = ref.watch(realtimeReviewsByIdProvider(reviewsId));
///     
///     return reviewsStream.when(
///       data: (reviews) => reviews != null
///         ? ReviewsDetail(reviews: reviews)
///         : const Text('Reviews not found'),
///       loading: () => const CircularProgressIndicator(),
///       error: (e, _) => ErrorWidget.withDetails(message: e.toString()),
///     );
///   }
/// }
/// ```
final realtimeReviewsByIdProvider = StreamProvider.family<ReviewsModel?, String>((ref, id) {
  final repository = ref.watch(reviewsRepositoryProvider);
  
  AppLogger.debug('Starting realtime stream for reviews ID: $id', loggerName: 'Provider');
  
  return repository
      .streamById(id)
      .handleError((error, stackTrace) {
        AppLogger.error(
          'Realtime stream error for reviews ID $id: $error',
          loggerName: 'Provider',
          error: error,
          stackTrace: stackTrace,
        );
...
/// Realtime provider for filtered reviews records with client-side filtering
/// 
...
/// 
/// Note: For large tables, consider using the unfiltered realtimeReviewsProvider
/// and filtering in your widget to reduce processing overhead.
/// 
/// Example usage:
/// ```dart
/// class ActiveReviewsWidget extends ConsumerWidget {
///   Widget build(BuildContext context, WidgetRef ref) {
///     // Create stable filter reference to avoid provider recreation
///     final activeFilter = {'status': 'active'};
///     final activeReviewsStream = ref.watch(
///       realtimeFilteredReviewsProvider(activeFilter)
///     );
///     
///     return activeReviewsStream.when(
///       data: (activeReviews) => Column(
///         children: activeReviews
///           .map((reviews) => ReviewsCard(
///             key: ValueKey('active-reviews-${reviews.id}'),
///             reviews: reviews,
///           ))
///           .toList(),
///       ),
...
/// }
/// ```
final realtimeFilteredReviewsProvider = StreamProvider.family<List<ReviewsModel>, Map<String, dynamic>>((ref, filters) {
  final repository = ref.watch(reviewsRepositoryProvider);
  
  // Create a readable filter description for logging
  final filterDesc = filters.entries.map((e) => '${e.key}=${e.value}').join(', ');
  AppLogger.debug('Starting filtered realtime stream for reviews with filters: $filterDesc', loggerName: 'Provider');
  
  return repository
      .streamWhere(filters)
      .handleError((error, stackTrace) {
        AppLogger.error(
          'Filtered realtime stream error for reviews (filters: $filterDesc): $error',
          loggerName: 'Provider',
          error: error,
          stackTrace: stackTrace,
        );
...
/// Example usage:
/// ```dart
/// class ReliableReviewsWidget extends ConsumerWidget {
///   Widget build(BuildContext context, WidgetRef ref) {
///     final reviewsData = ref.watch(hybridReviewsProvider);
///     
///     return reviewsData.when(
///       data: (reviews) => ReviewsList(reviews: reviews),
///       loading: () => const ReviewsSkeleton(),
///       error: (e, _) => ReviewsErrorView(error: e),
///     );
///   }
/// }
/// ```
final hybridReviewsProvider = StreamProvider<List<ReviewsModel>>((ref) {
  final repository = ref.watch(reviewsRepositoryProvider);
  
  AppLogger.debug('Starting hybrid realtime/cached stream for reviews', loggerName: 'Provider');
  
  return repository
      .streamAll()
      .handleError((error, stackTrace) async* {
        AppLogger.warning(
          'Realtime failed for reviews, falling back to cached data: $error',
          loggerName: 'Provider',
          error: error,
          stackTrace: stackTrace,
        );
        ...
        try {
          ...
          AppLogger.debug('Successfully fell back to cached data for reviews (${fallbackData.length} records)', loggerName: 'Provider');
          yield fallbackData;
        } catch (fallbackError, fallbackStack) {
          AppLogger.error(
            'Both realtime and fallback failed for reviews: $fallbackError',
            loggerName: 'Provider',
            error: fallbackError,
...
/// Notifier class that handles reviews operations
class ReviewsNotifier extends StateNotifier<AsyncValue<List<ReviewsModel>>> {
  ...
  Future<void> _loadInitialData() async {
    try {
      AppLogger.debug('Loading initial reviews data', loggerName: 'Provider');
      final results = await _repository.findAll();
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('ReviewsNotifier', 'Loaded initial data', details: '${results.length} records');
      }
    } catch (e, stackTrace) {
      if (mounted) {
        state = AsyncValue.error(e, stackTrace);
        ProviderLogging.logStateChange('ReviewsNotifier', 'Failed to load initial data', error: e, stackTrace: stackTrace);
      }
    }
  }

  /// Fetch all reviews with basic sorting and filtering
  Future<List<ReviewsModel>> fetchAll({
    String? orderBy,
    bool ascending = true,
    ...
    try {
      ProviderLogging.logStateChange('ReviewsNotifier', 'Fetching data', 
      ...
      if (cacheKey != null) {
        results = await AppCache().getOrFetch<List<ReviewsModel>>(
          cacheKey,
          () => _repository.findAll(
            orderBy: orderBy,
            ascending: ascending,
      ...
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('ReviewsNotifier', 'Data fetched', details: '${results.length} records');
      }
      ...
      final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'Reviews');
      throw AppException(message: errorMsg, originalError: e);
    }
  }
  ...
  Future<ReviewsModel?> getById(String id) async {
    try {
      ...
      ProviderLogging.logStateChange('ReviewsNotifier', 'Getting record by ID', details: 'id: $id');
  ...
  /// Create a new record
  Future<ReviewsModel> create(ReviewsModel model) async {
    try {
      ProviderLogging.logStateChange('ReviewsNotifier', 'Creating record');
      final result = await _repository.insert(model);
      
      // Clear any cached list results that might contain this entity
      AppLogger.debug('Clearing reviews list caches after create', loggerName: 'Provider');
      ...
      if (mounted) {
        ...
        ProviderLogging.logStateChange('ReviewsNotifier', 'Record created', details: 'id: ${result.id}');
      }
  ...
  Future<ReviewsModel?> update(ReviewsModel model) async {
    try {
      final modelId = model.id;
      if (modelId.isEmpty) {
        const message = 'Cannot update review without ID';
        ProviderLogging.logStateChange('ReviewsNotifier', 'Update failed', details: message);
        throw AppException(message: message);
      }
      
      ProviderLogging.logStateChange('ReviewsNotifier', 'Updating record', details: 'id: $modelId');
      final result = await _repository.update(model);
      
      // Clear related caches
      if (result != null) {
        // Clear both the specific ID cache and list caches
        final idCacheKey = 'reviews:id:$modelId';
      ...
      if (result != null && mounted && state.hasValue) {
        ...
        if (index >= 0) {
          ...
          ProviderLogging.logStateChange('ReviewsNotifier', 'Record updated', details: 'id: $modelId');
        }
      }
  ...
  Future<void> delete(String id) async {
    try {
      ProviderLogging.logStateChange('ReviewsNotifier', 'Deleting record', details: 'id: $id');
      await _repository.delete(id);
      
      // Clear cache entries for this ID
      final idCacheKey = 'reviews:id:$id';
      AppLogger.debug('Clearing cache for key: $idCacheKey', loggerName: 'Provider');
      AppCache().remove(idCacheKey);
      
      // Clear list caches that might contain this entity (using a prefix)
      final prefix = 'reviews:';
      ...
      if (mounted && state.hasValue) {
        ...
        ProviderLogging.logStateChange('ReviewsNotifier', 'Record deleted', details: 'id: $id');
      }
    } catch (e, stackTrace) {
      final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'Reviews');
      throw AppException(message: errorMsg, originalError: e);
  ...
  void _clearRelatedCaches(ReviewsModel entity) {
    final prefix = 'reviews:';
    AppLogger.debug('Clearing list caches with prefix: $prefix', loggerName: 'Provider');
  ...
  Future<void> refresh() async {
    try {
      ProviderLogging.logStateChange('ReviewsNotifier', 'Refreshing data');
      if (mounted) {
        state = const AsyncValue.loading();
      }
      ...
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('ReviewsNotifier', 'Data refreshed', details: '${results.length} records');
      }
    } catch (e, stackTrace) {
      if (mounted) {
        state = AsyncValue.error(e, stackTrace);
      }
      ProviderLogging.logStateChange('ReviewsNotifier', 'Refresh failed', error: e, stackTrace: stackTrace);
    }
  }
}
