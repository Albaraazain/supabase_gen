...
import '../repositories/job_tasks_repository.dart';
...
// Repository provider
final jobTasksRepositoryProvider = Provider<JobTasksRepository>((ref) {
  AppLogger.debug('Creating JobTasksRepository instance', loggerName: 'Provider');
  return JobTasksRepository(Supabase.instance.client);
});

// Main provider for managing job_tasks data
final jobTasksProvider = StateNotifierProvider<JobTasksNotifier, AsyncValue<List<JobTasksModel>>>((ref) {
  final repository = ref.watch(jobTasksRepositoryProvider);
  AppLogger.debug('Creating JobTasksNotifier', loggerName: 'Provider');
  return JobTasksNotifier(repository);
});
...
String _createCacheKey(Map<String, dynamic> filters) {
  ...
  return 'job_tasks:${parts.join(',')}';
}

// Provider to get a single job_tasks by ID with caching
final jobTasksByIdProvider = FutureProvider.family<JobTasksModel?, String>((ref, id) async {
  // Create a stable cache key for this ID lookup
  final cacheKey = 'job_tasks:id:$id';
  
  AppLogger.debug('jobTasksByIdProvider called with id: $id', loggerName: 'Provider');
  final repository = ref.watch(jobTasksRepositoryProvider);
  
  try {
    // Use the app cache to prevent redundant database calls
    final result = await AppCache().getOrFetch<JobTasksModel?>(
      cacheKey,
      () => repository.find(id),
    ...
    if (result == null) {
      AppLogger.warning('No job_task found with ID: $id', loggerName: 'Provider');
    } else {
      AppLogger.debug('Found job_task with ID: $id', loggerName: 'Provider');
    }
    
    return result;
  } catch (e, stackTrace) {
    final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'JobTasksById');
    throw AppException(message: errorMsg, originalError: e);
  }
});

// Provider to get filtered job_tasks with proper caching
final filteredJobTasksProvider = FutureProvider.family<List<JobTasksModel>, Map<String, dynamic>>((ref, filters) async {
  // Create a stable cache key from the filters
  final cacheKey = _createCacheKey(filters);
  
  AppLogger.debug('filteredJobTasksProvider called with key: $cacheKey', loggerName: 'Provider');
  final repository = ref.watch(jobTasksRepositoryProvider);
  ...
  try {
    ...
    final results = await AppCache().getOrFetch<List<JobTasksModel>>(
      cacheKey,
      () async {
    ...
    AppLogger.debug('filteredJobTasksProvider returned ${results.length} items for key: $cacheKey', loggerName: 'Provider');
    return results;
  } catch (e, stackTrace) {
    final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'FilteredJobTasks');
    throw AppException(message: errorMsg, originalError: e);
  }
});
...
/// Notifier class that handles job_tasks operations
class JobTasksNotifier extends StateNotifier<AsyncValue<List<JobTasksModel>>> {
  final JobTasksRepository _repository;

  JobTasksNotifier(this._repository) : super(const AsyncValue.loading()) {
  ...
  Future<void> _loadInitialData() async {
    try {
      AppLogger.debug('Loading initial job_tasks data', loggerName: 'Provider');
      final results = await _repository.findAll();
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('JobTasksNotifier', 'Loaded initial data', details: '${results.length} records');
      }
    } catch (e, stackTrace) {
      if (mounted) {
        state = AsyncValue.error(e, stackTrace);
        ProviderLogging.logStateChange('JobTasksNotifier', 'Failed to load initial data', error: e, stackTrace: stackTrace);
      }
    }
  }

  /// Fetch all job_tasks with basic sorting and filtering
  Future<List<JobTasksModel>> fetchAll({
    String? orderBy,
    bool ascending = true,
    ...
    try {
      ProviderLogging.logStateChange('JobTasksNotifier', 'Fetching data', 
      ...
      final List<JobTasksModel> results;
      if (cacheKey != null) {
        results = await AppCache().getOrFetch<List<JobTasksModel>>(
          cacheKey,
          () => _repository.findAll(
            orderBy: orderBy,
            ascending: ascending,
      ...
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('JobTasksNotifier', 'Data fetched', details: '${results.length} records');
      }
      ...
      final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'JobTasks');
      throw AppException(message: errorMsg, originalError: e);
    }
  }
  
  /// Get a single record by ID with caching
  Future<JobTasksModel?> getById(String id) async {
    try {
      // Create a stable cache key
      final cacheKey = 'job_tasks:id:$id';
      
      ProviderLogging.logStateChange('JobTasksNotifier', 'Getting record by ID', details: 'id: $id');
  ...
  /// Create a new record
  Future<JobTasksModel> create(JobTasksModel model) async {
    try {
      ProviderLogging.logStateChange('JobTasksNotifier', 'Creating record');
      final result = await _repository.insert(model);
      
      // Clear any cached list results that might contain this entity
      AppLogger.debug('Clearing job_tasks list caches after create', loggerName: 'Provider');
      ...
      if (mounted) {
        ...
        ProviderLogging.logStateChange('JobTasksNotifier', 'Record created', details: 'id: ${result.id}');
      }
  ...
  Future<JobTasksModel?> update(JobTasksModel model) async {
    try {
      ...
      if (modelId.isEmpty) {
        const message = 'Cannot update job_task without ID';
        ProviderLogging.logStateChange('JobTasksNotifier', 'Update failed', details: message);
        throw AppException(message: message);
      }
      
      ProviderLogging.logStateChange('JobTasksNotifier', 'Updating record', details: 'id: $modelId');
      final result = await _repository.update(model);
      
      // Clear related caches
      if (result != null) {
        // Clear both the specific ID cache and list caches
        final idCacheKey = 'job_tasks:id:$modelId';
      ...
      if (result != null && mounted && state.hasValue) {
        ...
        if (index >= 0) {
          ...
          ProviderLogging.logStateChange('JobTasksNotifier', 'Record updated', details: 'id: $modelId');
        }
      }
  ...
  Future<void> delete(String id) async {
    try {
      ProviderLogging.logStateChange('JobTasksNotifier', 'Deleting record', details: 'id: $id');
      await _repository.delete(id);
      
      // Clear cache entries for this ID
      final idCacheKey = 'job_tasks:id:$id';
      AppLogger.debug('Clearing cache for key: $idCacheKey', loggerName: 'Provider');
      AppCache().remove(idCacheKey);
      
      // Clear list caches that might contain this entity (using a prefix)
      final prefix = 'job_tasks:';
      ...
      if (mounted && state.hasValue) {
        ...
        ProviderLogging.logStateChange('JobTasksNotifier', 'Record deleted', details: 'id: $id');
      }
    } catch (e, stackTrace) {
      final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'JobTasks');
      throw AppException(message: errorMsg, originalError: e);
  ...
  void _clearRelatedCaches(JobTasksModel entity) {
    final prefix = 'job_tasks:';
    AppLogger.debug('Clearing list caches with prefix: $prefix', loggerName: 'Provider');
  ...
  Future<void> refresh() async {
    try {
      ProviderLogging.logStateChange('JobTasksNotifier', 'Refreshing data');
      if (mounted) {
        state = const AsyncValue.loading();
      }
      ...
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('JobTasksNotifier', 'Data refreshed', details: '${results.length} records');
      }
    } catch (e, stackTrace) {
      if (mounted) {
        state = AsyncValue.error(e, stackTrace);
      }
      ProviderLogging.logStateChange('JobTasksNotifier', 'Refresh failed', error: e, stackTrace: stackTrace);
    }
  }
}
