...
final servicesRepositoryProvider = Provider<ServicesRepository>((ref) {
  AppLogger.debug('Creating ServicesRepository instance', loggerName: 'Provider');
  return ServicesRepository(Supabase.instance.client);
});

// Main provider for managing services data
final servicesProvider = StateNotifierProvider<ServicesNotifier, AsyncValue<List<ServicesModel>>>((ref) {
  final repository = ref.watch(servicesRepositoryProvider);
  AppLogger.debug('Creating ServicesNotifier', loggerName: 'Provider');
...
// Provider to get a single services by ID with caching
final servicesByIdProvider = FutureProvider.family<ServicesModel?, String>((ref, id) async {
  // Create a stable cache key for this ID lookup
  final cacheKey = 'services:id:$id';
  
  AppLogger.debug('servicesByIdProvider called with id: $id', loggerName: 'Provider');
  final repository = ref.watch(servicesRepositoryProvider);
  
  try {
    // Use the app cache to prevent redundant database calls
    final result = await AppCache().getOrFetch<ServicesModel?>(
      cacheKey,
      () => repository.find(id),
    ...
    if (result == null) {
      AppLogger.warning('No servic found with ID: $id', loggerName: 'Provider');
    } else {
      AppLogger.debug('Found servic with ID: $id', loggerName: 'Provider');
    }
    
    return result;
  } catch (e, stackTrace) {
    final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'ServicesById');
    throw AppException(message: errorMsg, originalError: e);
  }
});

// Provider to get filtered services with proper caching
final filteredServicesProvider = FutureProvider.family<List<ServicesModel>, Map<String, dynamic>>((ref, filters) async {
  // Create a stable cache key from the filters
  final cacheKey = _createCacheKey(filters);
  
  AppLogger.debug('filteredServicesProvider called with key: $cacheKey', loggerName: 'Provider');
  final repository = ref.watch(servicesRepositoryProvider);
  ...
  try {
    ...
    final results = await AppCache().getOrFetch<List<ServicesModel>>(
      cacheKey,
      () async {
    ...
    AppLogger.debug('filteredServicesProvider returned ${results.length} items for key: $cacheKey', loggerName: 'Provider');
    return results;
  } catch (e, stackTrace) {
    final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'FilteredServices');
    throw AppException(message: errorMsg, originalError: e);
  }
});
...
/// Notifier class that handles services operations
class ServicesNotifier extends StateNotifier<AsyncValue<List<ServicesModel>>> {
  ...
  Future<void> _loadInitialData() async {
    try {
      AppLogger.debug('Loading initial services data', loggerName: 'Provider');
      final results = await _repository.findAll();
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('ServicesNotifier', 'Loaded initial data', details: '${results.length} records');
      }
    } catch (e, stackTrace) {
      if (mounted) {
        state = AsyncValue.error(e, stackTrace);
        ProviderLogging.logStateChange('ServicesNotifier', 'Failed to load initial data', error: e, stackTrace: stackTrace);
      }
    }
  }

  /// Fetch all services with basic sorting and filtering
  Future<List<ServicesModel>> fetchAll({
    String? orderBy,
    bool ascending = true,
    ...
    try {
      ProviderLogging.logStateChange('ServicesNotifier', 'Fetching data', 
      ...
      if (cacheKey != null) {
        results = await AppCache().getOrFetch<List<ServicesModel>>(
          cacheKey,
          () => _repository.findAll(
            orderBy: orderBy,
            ascending: ascending,
      ...
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('ServicesNotifier', 'Data fetched', details: '${results.length} records');
      }
      ...
      final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'Services');
      throw AppException(message: errorMsg, originalError: e);
    }
  }
  
  /// Get a single record by ID with caching
  Future<ServicesModel?> getById(String id) async {
    try {
      // Create a stable cache key
      final cacheKey = 'services:id:$id';
      
      ProviderLogging.logStateChange('ServicesNotifier', 'Getting record by ID', details: 'id: $id');
  ...
  /// Create a new record
  Future<ServicesModel> create(ServicesModel model) async {
    try {
      ProviderLogging.logStateChange('ServicesNotifier', 'Creating record');
      final result = await _repository.insert(model);
      
      // Clear any cached list results that might contain this entity
      AppLogger.debug('Clearing services list caches after create', loggerName: 'Provider');
      ...
      if (mounted) {
        ...
        ProviderLogging.logStateChange('ServicesNotifier', 'Record created', details: 'id: ${result.id}');
      }
  ...
  Future<ServicesModel?> update(ServicesModel model) async {
    try {
      ...
      if (modelId.isEmpty) {
        const message = 'Cannot update servic without ID';
        ProviderLogging.logStateChange('ServicesNotifier', 'Update failed', details: message);
        throw AppException(message: message);
      }
      
      ProviderLogging.logStateChange('ServicesNotifier', 'Updating record', details: 'id: $modelId');
      final result = await _repository.update(model);
      
      // Clear related caches
      if (result != null) {
        // Clear both the specific ID cache and list caches
        final idCacheKey = 'services:id:$modelId';
      ...
      if (result != null && mounted && state.hasValue) {
        ...
        if (index >= 0) {
          ...
          ProviderLogging.logStateChange('ServicesNotifier', 'Record updated', details: 'id: $modelId');
        }
      }
  ...
  Future<void> delete(String id) async {
    try {
      ProviderLogging.logStateChange('ServicesNotifier', 'Deleting record', details: 'id: $id');
      await _repository.delete(id);
      
      // Clear cache entries for this ID
      final idCacheKey = 'services:id:$id';
      AppLogger.debug('Clearing cache for key: $idCacheKey', loggerName: 'Provider');
      AppCache().remove(idCacheKey);
      
      // Clear list caches that might contain this entity (using a prefix)
      final prefix = 'services:';
      ...
      if (mounted && state.hasValue) {
        ...
        ProviderLogging.logStateChange('ServicesNotifier', 'Record deleted', details: 'id: $id');
      }
    } catch (e, stackTrace) {
      final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'Services');
      throw AppException(message: errorMsg, originalError: e);
  ...
  void _clearRelatedCaches(ServicesModel entity) {
    final prefix = 'services:';
    AppLogger.debug('Clearing list caches with prefix: $prefix', loggerName: 'Provider');
  ...
  Future<void> refresh() async {
    try {
      ProviderLogging.logStateChange('ServicesNotifier', 'Refreshing data');
      if (mounted) {
        state = const AsyncValue.loading();
      }
      ...
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('ServicesNotifier', 'Data refreshed', details: '${results.length} records');
      }
    } catch (e, stackTrace) {
      if (mounted) {
        state = AsyncValue.error(e, stackTrace);
      }
      ProviderLogging.logStateChange('ServicesNotifier', 'Refresh failed', error: e, stackTrace: stackTrace);
    }
  }
}
