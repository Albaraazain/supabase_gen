...
final paymentMethodsRepositoryProvider = Provider<PaymentMethodsRepository>((ref) {
  AppLogger.debug('Creating PaymentMethodsRepository instance', loggerName: 'Provider');
  return PaymentMethodsRepository(Supabase.instance.client);
});

// Main provider for managing payment_methods data
final paymentMethodsProvider = StateNotifierProvider<PaymentMethodsNotifier, AsyncValue<List<PaymentMethodsModel>>>((ref) {
  final repository = ref.watch(paymentMethodsRepositoryProvider);
  AppLogger.debug('Creating PaymentMethodsNotifier', loggerName: 'Provider');
...
String _createCacheKey(Map<String, dynamic> filters) {
  ...
  return 'payment_methods:${parts.join(',')}';
}

// Provider to get a single payment_methods by ID with caching
final paymentMethodsByIdProvider = FutureProvider.family<PaymentMethodsModel?, String>((ref, id) async {
  // Create a stable cache key for this ID lookup
  final cacheKey = 'payment_methods:id:$id';
  
  AppLogger.debug('paymentMethodsByIdProvider called with id: $id', loggerName: 'Provider');
  final repository = ref.watch(paymentMethodsRepositoryProvider);
  
  try {
    // Use the app cache to prevent redundant database calls
    final result = await AppCache().getOrFetch<PaymentMethodsModel?>(
      cacheKey,
      () => repository.find(id),
    ...
    if (result == null) {
      AppLogger.warning('No payment_method found with ID: $id', loggerName: 'Provider');
    } else {
      AppLogger.debug('Found payment_method with ID: $id', loggerName: 'Provider');
    }
    
    return result;
  } catch (e, stackTrace) {
    final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'PaymentMethodsById');
    throw AppException(message: errorMsg, originalError: e);
  }
});

// Provider to get filtered payment_methods with proper caching
final filteredPaymentMethodsProvider = FutureProvider.family<List<PaymentMethodsModel>, Map<String, dynamic>>((ref, filters) async {
  // Create a stable cache key from the filters
  final cacheKey = _createCacheKey(filters);
  
  AppLogger.debug('filteredPaymentMethodsProvider called with key: $cacheKey', loggerName: 'Provider');
  final repository = ref.watch(paymentMethodsRepositoryProvider);
  ...
  try {
    ...
    final results = await AppCache().getOrFetch<List<PaymentMethodsModel>>(
      cacheKey,
      () async {
    ...
    AppLogger.debug('filteredPaymentMethodsProvider returned ${results.length} items for key: $cacheKey', loggerName: 'Provider');
    return results;
  } catch (e, stackTrace) {
    final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'FilteredPaymentMethods');
    throw AppException(message: errorMsg, originalError: e);
  }
});
...
/// Notifier class that handles payment_methods operations
class PaymentMethodsNotifier extends StateNotifier<AsyncValue<List<PaymentMethodsModel>>> {
  final PaymentMethodsRepository _repository;

  PaymentMethodsNotifier(this._repository) : super(const AsyncValue.loading()) {
  ...
  Future<void> _loadInitialData() async {
    try {
      AppLogger.debug('Loading initial payment_methods data', loggerName: 'Provider');
      final results = await _repository.findAll();
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('PaymentMethodsNotifier', 'Loaded initial data', details: '${results.length} records');
      }
    } catch (e, stackTrace) {
      if (mounted) {
        state = AsyncValue.error(e, stackTrace);
        ProviderLogging.logStateChange('PaymentMethodsNotifier', 'Failed to load initial data', error: e, stackTrace: stackTrace);
      }
    }
  }

  /// Fetch all payment_methods with basic sorting and filtering
  Future<List<PaymentMethodsModel>> fetchAll({
    String? orderBy,
    bool ascending = true,
    ...
    try {
      ProviderLogging.logStateChange('PaymentMethodsNotifier', 'Fetching data', 
      ...
      final List<PaymentMethodsModel> results;
      if (cacheKey != null) {
        results = await AppCache().getOrFetch<List<PaymentMethodsModel>>(
          cacheKey,
          () => _repository.findAll(
            orderBy: orderBy,
            ascending: ascending,
      ...
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('PaymentMethodsNotifier', 'Data fetched', details: '${results.length} records');
      }
      ...
      final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'PaymentMethods');
      throw AppException(message: errorMsg, originalError: e);
    }
  }
  ...
  Future<PaymentMethodsModel?> getById(String id) async {
    try {
      ...
      ProviderLogging.logStateChange('PaymentMethodsNotifier', 'Getting record by ID', details: 'id: $id');
  ...
  /// Create a new record
  Future<PaymentMethodsModel> create(PaymentMethodsModel model) async {
    try {
      ProviderLogging.logStateChange('PaymentMethodsNotifier', 'Creating record');
      final result = await _repository.insert(model);
      
      // Clear any cached list results that might contain this entity
      AppLogger.debug('Clearing payment_methods list caches after create', loggerName: 'Provider');
      ...
      if (mounted) {
        ...
        ProviderLogging.logStateChange('PaymentMethodsNotifier', 'Record created', details: 'id: ${result.id}');
      }
  ...
  Future<PaymentMethodsModel?> update(PaymentMethodsModel model) async {
    try {
      final modelId = model.id;
      if (modelId.isEmpty) {
        const message = 'Cannot update payment_method without ID';
        ProviderLogging.logStateChange('PaymentMethodsNotifier', 'Update failed', details: message);
        throw AppException(message: message);
      }
      
      ProviderLogging.logStateChange('PaymentMethodsNotifier', 'Updating record', details: 'id: $modelId');
      final result = await _repository.update(model);
      
      // Clear related caches
      if (result != null) {
        // Clear both the specific ID cache and list caches
        final idCacheKey = 'payment_methods:id:$modelId';
      ...
      if (result != null && mounted && state.hasValue) {
        ...
        if (index >= 0) {
          ...
          ProviderLogging.logStateChange('PaymentMethodsNotifier', 'Record updated', details: 'id: $modelId');
        }
      }
  ...
  Future<void> delete(String id) async {
    try {
      ProviderLogging.logStateChange('PaymentMethodsNotifier', 'Deleting record', details: 'id: $id');
      await _repository.delete(id);
      
      // Clear cache entries for this ID
      final idCacheKey = 'payment_methods:id:$id';
      AppLogger.debug('Clearing cache for key: $idCacheKey', loggerName: 'Provider');
      AppCache().remove(idCacheKey);
      
      // Clear list caches that might contain this entity (using a prefix)
      final prefix = 'payment_methods:';
      ...
      if (mounted && state.hasValue) {
        ...
        ProviderLogging.logStateChange('PaymentMethodsNotifier', 'Record deleted', details: 'id: $id');
      }
    } catch (e, stackTrace) {
      final errorMsg = AppExceptionHandler.handleException(e, stackTrace, context: 'PaymentMethods');
      throw AppException(message: errorMsg, originalError: e);
  ...
  void _clearRelatedCaches(PaymentMethodsModel entity) {
    final prefix = 'payment_methods:';
    AppLogger.debug('Clearing list caches with prefix: $prefix', loggerName: 'Provider');
  ...
  Future<void> refresh() async {
    try {
      ProviderLogging.logStateChange('PaymentMethodsNotifier', 'Refreshing data');
      if (mounted) {
        state = const AsyncValue.loading();
      }
      ...
      if (mounted) {
        state = AsyncValue.data(results);
        ProviderLogging.logStateChange('PaymentMethodsNotifier', 'Data refreshed', details: '${results.length} records');
      }
    } catch (e, stackTrace) {
      if (mounted) {
        state = AsyncValue.error(e, stackTrace);
      }
      ProviderLogging.logStateChange('PaymentMethodsNotifier', 'Refresh failed', error: e, stackTrace: stackTrace);
    }
  }
}
