                ...
                .map((column) {
                  if (column is Map<String, dynamic>) {
                    try {
                      ...
                      final defaultValue = column['col_default'] as String?;
                      final type = column['col_type'] as String;
                      return ColumnInfo(
                        ...
                        isAutoGenerated: ColumnInfo.isAutoGeneratedByDefault(defaultValue, type, false),
                      );
                    } catch (e) {
      ...
      if (response.statusCode == 200) {
        ...
        if (columnTypes.isNotEmpty) {
          ...
          columnTypes.forEach((name, type) {
            final isPrimaryKey = name == 'id';
            columns.add(
              ColumnInfo(
                name: name,
                type: type,
                isNullable: true, // Assume nullable by default
                isPrimaryKey: isPrimaryKey, // Assume 'id' is primary key
                isUnique: name == 'id', // Assume 'id' is unique
                defaultValue: null,
                comment: null,
                maxLength: null,
                ordinalPosition: null,
                isAutoGenerated: ColumnInfo.isAutoGeneratedByDefault(null, type, isPrimaryKey),
              ),
            );
          });
          return columns;
        ...
        switch (constraint.type.toUpperCase()) {
          case 'PRIMARY KEY':
            ...
            updatedColumn = ColumnInfo(
              ...
              isAutoGenerated: ColumnInfo.isAutoGeneratedByDefault(column.defaultValue, column.type, true),
            );
            break;
          ...
          case 'FOREIGN KEY':
            ...
            updatedColumn = ColumnInfo(
              ...
              maxLength: column.maxLength,
              ordinalPosition: column.ordinalPosition,
              isAutoGenerated: ColumnInfo.isAutoGeneratedByDefault(column.defaultValue, column.type, column.isPrimaryKey),
            );
            break;
...
AS \$\$
  ...
  WHERE 
    ...
    AND NOT (p.proname LIKE '\_%' OR p.proname LIKE 'pg\_%' OR p.proname LIKE 'supabase\_%' OR p.proname LIKE 'auth.%' OR p.proname LIKE 'storage.%')  -- Exclude internal function names
...
''';
  ...
  List<ColumnInfo> _createImprovedMinimalColumns(String tableName) {
    ...
    const defaultValue = 'uuid_generate_v4()';
    const type = 'uuid';
    columns.add(
      ColumnInfo(
        name: 'id',
        ...
        foreignKey: null,
        foreignTable: null,
        isAutoGenerated: ColumnInfo.isAutoGeneratedByDefault(defaultValue, type, true),
      ),
    );

    // Add common timestamps
    const createdAtDefaultValue = 'now()';
    const createdAtType = 'timestamp with time zone';
    columns.add(
      ColumnInfo(
        name: 'created_at',
        type: createdAtType,
        ...
        foreignKey: null,
        foreignTable: null,
        isAutoGenerated: ColumnInfo.isAutoGeneratedByDefault(createdAtDefaultValue, createdAtType, false),
      ),
    );
      ...
      if (parts.length == 2) {
        ...
        columns.add(
          ColumnInfo(
            ...
            isAutoGenerated: ColumnInfo.isAutoGeneratedByDefault(null, firstEntityType, false),
          ),
        );

        const secondEntityType = 'uuid';
        columns.add(
          ColumnInfo(
            name: '${secondEntity}_id',
            type: secondEntityType,
            isNullable: false,
            isPrimaryKey: false,
            isUnique: false,
            defaultValue: null,
            ...
            isAutoGenerated: ColumnInfo.isAutoGeneratedByDefault(null, secondEntityType, false),
          ),
        );
      }
    }
  ...
  Future<List<ColumnInfo>> _readTableColumnsLocal(
    ...
    final columnsResult = await _connection!.execute(
      'SELECT DISTINCT ON (c.column_name) c.column_name, '
      'CASE '
      '  WHEN c.data_type = \'ARRAY\' THEN '
      '    CASE '
      '      WHEN c.udt_name LIKE \'_%\' THEN SUBSTRING(c.udt_name FROM 2) || \'[]\' '
    ...
    for (final columnRow in columnsResult) {
      ...
      columns.add(
        ColumnInfo(
          ...
          isAutoGenerated: ColumnInfo.isAutoGeneratedByDefault(defaultValue, dataType, isPrimaryKey),
        ),
      );
    }
