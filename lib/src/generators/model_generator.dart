...
''';
          ...
          if (column.defaultValue != null) {
            ...
            if (defaultValue.contains('(') ||
              ...
              if (!isNullable) {
                ...
                // Convert database defaults to appropriate Dart values
                if (lowerType == 'uuid' ||
                    defaultValue.contains('uuid_generate')) {
                  return 'this.$name = ""';
                } else if (dartDefault != null) {
  ...
  Map<String, dynamic> toJson() {
    ...
    final toJsonForInsertMappers = table.columns
        .where((column) => !column.excludeFromSerialization && !_isAutoGeneratedField(column))
        .map((column) => _generateToJsonMapper(column))
        .join('\n      ');
...
''';
  ...
  /// Check if a column is an auto-generated field that should be excluded from inserts
  bool _isAutoGeneratedField(ColumnInfo column) {
    final name = column.name.toLowerCase();
    final type = column.type.toLowerCase();
    
    // Check for UUID primary keys with default generation
    if (column.isPrimaryKey && 
        type == 'uuid' && 
        (column.defaultValue?.contains('uuid_generate') ?? false)) {
      return true;
    }
    
    // Check for auto-timestamp fields
    if ((name == 'created_at' || name == 'updated_at') &&
        (type.contains('timestamp') || type.contains('date')) &&
        (column.defaultValue?.contains('now()') ?? false)) {
      return true;
    }
