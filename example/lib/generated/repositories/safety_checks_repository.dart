...
class SafetyChecksRepository extends BaseRepository<SafetyChecksModel> {
  ...
  Future<SafetyChecksModel?> update(SafetyChecksModel model) async {
    if (model.checkId == null) {
      throw ArgumentError("Primary key check_id cannot be null for update operation");
    }
  ...
  /// based on the performed_by foreign key
  Future<List<UsersModel>> findByPerformedBy(String? performedBy) async {
    final queryBuilder = client
        .from('users')
        .select()
        .eq('user_id', performedBy as Object);
