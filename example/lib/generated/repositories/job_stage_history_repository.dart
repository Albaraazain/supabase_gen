...
class JobStageHistoryRepository extends BaseRepository<JobStageHistoryModel> {
  ...
  Future<JobStageHistoryModel?> update(JobStageHistoryModel model) async {
    if (model.historyId == null) {
      throw ArgumentError("Primary key history_id cannot be null for update operation");
    }
  ...
  /// based on the changed_by foreign key
  Future<List<UsersModel>> findByChangedBy(String? changedBy) async {
    final queryBuilder = client
        .from('users')
        .select()
        .eq('user_id', changedBy as Object);
