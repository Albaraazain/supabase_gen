...
class SitePhotosRepository extends BaseRepository<SitePhotosModel> {
  ...
  Future<SitePhotosModel?> update(SitePhotosModel model) async {
    if (model.photoId == null) {
      throw ArgumentError("Primary key photo_id cannot be null for update operation");
    }
  ...
  /// based on the uploaded_by foreign key
  Future<List<UsersModel>> findByUploadedBy(String? uploadedBy) async {
    final queryBuilder = client
        .from('users')
        .select()
        .eq('user_id', uploadedBy as Object);
